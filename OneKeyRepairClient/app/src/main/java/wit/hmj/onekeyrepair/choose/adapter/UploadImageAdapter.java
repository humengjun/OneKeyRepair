package wit.hmj.onekeyrepair.choose.adapter;
...
public class UploadImageAdapter extends BaseAdapter {
	...
	public LinkedList<String> imagePathList;
	...
	public void update(LinkedList<String> imagePathList){
		...
		if(imagePathList.get(imagePathList.size()-1)==null){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {//创建ImageView
			...
			iv_image.setLayoutParams(new AbsListView.LayoutParams(ImageUtils.getWidth(context) / 4 - 20, ImageUtils.getWidth(context) / 4 - 20) );
		...
		if(getItem(position) == null ){//图片地址为空时设置默认图片
			iv_image.setImageResource(R.mipmap.tjtp);
		}else{
			//获取图片缩略图，避免OOM
			Bitmap bitmap = ImageUtils.getImageThumbnail((String)getItem(position), ImageUtils.getWidth(context) / 4 - 20, ImageUtils.getWidth(context) / 4 - 20);
