 ...
 */
@WebServlet("/PersonInfo_Servlet")
public class PersonInfo_Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * Default constructor.
	 */
	public PersonInfo_Servlet() {
		...
		try {
			...
			if (resultSet.next()) {
				...
				String jsonString = GenerateJsonString
						.getPersonInfoJsonString((new PersonInfoJson("success",
								new PersonInfoJson.DataBean(name, phone), 1)));
				System.out.println(jsonString);
				out.write(jsonString);
			} else {
				String jsonString = GenerateJsonString
						.getErrorJsonString("用户不存在请重新登录");
				out.write(jsonString);
			}
		} catch (Exception e) {
			// TODO 自动生成的 catch 块
